...
class CartService  {
    ...
    private readonly REMOVE_FROM_CART = import.meta.env.VITE_USER_REMOVE_FROM_CART;
    ...
    async getCartTotalItems(): Promise<number> {
        ...
        return cart.reduce((acc: number, item: ICartItem) => acc + item.quantity, 0); // Correction ici
    }
    ...
    async addToCart(productId: number, productName: string, productImage: string, productPrice: number) {
        ...
        if (existingItem) {
            ...
            const newItem: ICartItem = {
                product: { id: productId, name: productName, image: productImage, price: productPrice },
                id: productId,
                name: productName,
                image: productImage,
                price: productPrice,
        ...
        if (!user) {
            ...
            const productToSend = cart.find((item: ICartItem) => item.id === productId);
        ...
        if (user) {
            ...
            await axios.post(`${this.BACKEND_URL}${this.ADD_TO_CART}`, {
                productId: productId,
                cart: cart
            }, {
                headers: authHeader()
    ...
    async removeFromCart(productId: number) {
        ...
        if (!user) {
            ...
            try {
                await axios.delete(`${this.BACKEND_URL}${this.REMOVE_FROM_CART}/${productId}`, {
                    headers: authHeader()
                });
            } catch (error) {
                console.error(error);
